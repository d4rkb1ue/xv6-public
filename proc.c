...
int thread_create(void(*fcn)(void*), void *arg, void *stack)
{
	...
	//cprintf("in proc.c thread_create, curproc: [%d], fcn: %d, arg: %d, stack: %d\n", curproc->pid, (uint)fcn, (uint)arg, (uint)stack);
	...
	//cprintf("new thread created_pid = [%d]\n", np->pid);

	// do not clone, just copy the address
...
int thread_join(void)
{
	...
	for(;;){
		...
		havekids = 0;
		for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
			// also check if the pgdir is the same, thus determine whether is a thread instead of a child process
			if(p->parent != curproc || p->pgdir != curproc->pgdir)
				continue;
			havekids = 1;
			if(p->state == ZOMBIE){
